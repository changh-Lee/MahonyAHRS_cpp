...
m_simulate = horzcat(x_m, y_m, z_m);
writematrix(m_simulate, 'simulated_points.csv');

% select z-plane
z_max = max(z_m);
z_min = min(z_m);
z_span = z_max - z_min;
z_target = z_min + z_span*0.5; 
d = 5;
distances = abs(m_simulate(:, 3) - z_target);
selected_points = m_simulate(distances <= d, :);
writematrix(selected_points, 'plane_points.csv');
...
% z-plane 
figure;
scatter3(selected_points(:,1), selected_points(:,2), selected_points(:,3), 'fill', 'MarkerFaceColor', 'green'); hold on; 
plot_ellipsoid(v); 
title({'Selected magnetometer data', '(Ellipsoid fit)'});
xlabel('X-axis'); ylabel('Y-axis'); zlabel('Z-axis');
axis equal;
